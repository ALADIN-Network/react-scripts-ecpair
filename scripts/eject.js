...
inquirer
  ...
  .then(answer => {
    ...
    if (appPackage.devDependencies) {
      // We used to put react-scripts-ecpair in devDependencies
    ...
    if (fs.existsSync(paths.appTypeDeclarations)) {
      try {
        ...
        // Remove react-scripts-ecpair reference since they're getting a copy of the types in their project
        content =
          content
            // Remove react-scripts-ecpair types
            .replace(
              /^\s*\/\/\/\s*<reference\s+types.+?"react-scripts-ecpair".*\/>.*(?:\n|$)/gm,
              ''
            )
            .trim() + os.EOL;
    ...
    if (ownPath.indexOf(appPath) === 0) {
      try {
        // remove react-scripts-ecpair and react-scripts-ecpair binaries from app node_modules
        Object.keys(ownPackage.bin).forEach(binKey => {
    ...
    if (fs.existsSync(paths.yarnLockFile)) {
      const windowsCmdFilePath = path.join(
        ...
        'node_modules',
        '.bin',
        'react-scripts-ecpair.cmd'
      );
      let windowsCmdFileContent;
      if (process.platform === 'win32') {
        // https://github.com/facebook/create-react-app/pull/3806#issuecomment-357781035
        // Yarn is diligent about cleaning up after itself, but this causes the react-scripts-ecpair.cmd file
